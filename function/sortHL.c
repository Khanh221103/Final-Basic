...
void sortHigh(struct Information students[1000], int s) {
    printf("Table after sorting GPA from high to low.\n");
    int outer, inner;
    struct Information temp;
    for(outer = 0; outer < s; outer++) {
        for(inner = outer + 1; inner < s; inner++) {
            if(students[outer].GPA < students[inner].GPA) {
                temp = students[outer];
                students[outer] = students[inner];
                students[inner] = temp;
            }
        }
    }
